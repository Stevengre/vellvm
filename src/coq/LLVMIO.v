...
Definition Trace X := state -> (M IO (state * X)).
Hint Unfold Trace.

Definition Trace_map {A B} (f : A -> B) (s : Trace A) : Trace B :=
  fun st => fmap (fun '(st, x) => (st, f x)) (s st).
...
Definition Trace_ret {A} (x:A) : Trace A :=
  ...
  (*fun st '(st', t) => st = st' /\ t = Ret x.  (* SAZ: should relax the equivalence *) *)
Definition Trace_bind {A} {B} (t:Trace A) (f : A -> Trace B) : Trace B :=
  fun st => (t st) ≫= (fun '(st', a) => f a st').
